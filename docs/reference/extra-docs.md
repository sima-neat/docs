...
New to the DevKit? Start here — the interactive guide walks you through first boot before you dig into the rest of the documentation.
...
Understand the platform and confirm a product fits your application before diving into board-level detail.
...
| **Modalix PCIe HHHL Hardware Reference** | Hardware reference for the half-height/half-length (HHHL) Modalix PCIe card — connector and power details, mechanical envelope, and host-integration requirements. Review it to evaluate the card for a host-attached inference system. | [Download PDF](https://docs.sima.ai/pkg_downloads/datasheets_product_briefs/SiMa_Modalix_PHHHL_Manual_Rev1.00_2-21-2026.pdf) |
...
Designing the MLSoC Modalix die directly onto your PCB (chip-on-board) instead of using the pre-integrated SoM? These cover the high-speed routing, memory tuning, and a worked reference design that a bare-die layout needs to get right.
...
| **Modalix Early Access DevKit Design Package** | Schematics and design collateral for the Early Access DevKit — a worked chip-on-board reference design for laying out your own board around the bare SoC. | *Contact your SiMa.ai representative* |
| **Modalix PCIe HHHL Design Package** | Schematics and design collateral for the PCIe HHHL card, useful as a chip-on-board reference when adapting or extending the design. | *Contact your SiMa.ai representative* |
...
SiMa.ai provides a dedicated SoM Thermal Design Guide for designing a Modalix SoM thermal solution. Pair it with the electrical, thermal, and mechanical limits in the datasheets and hardware references listed elsewhere on this page when sizing power delivery, selecting heatsinks, and validating cooling.

| Document | Description | Download |
| --- | --- | --- |
| **Modalix SoM Thermal Design Guide** | The dedicated reference for designing a Modalix SoM thermal solution — power-dissipation and thermal budgets, recommended heatsink and airflow configurations, junction-temperature management, and measurement and validation procedures. The primary guide when designing cooling for a custom board or enclosure. | *Contact your SiMa.ai representative* |
