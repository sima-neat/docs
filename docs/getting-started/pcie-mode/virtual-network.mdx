...
2. `ifconfig` output on the SiMa.ai DevKit:
   ...
   ```console
   veth-simaai: flags=4163<UP,BROADCAST,RUNNING,MULTICAST>  mtu 65536
   ...
   :::note
   Older pre-eLxr builds name this interface `eth2`. If `veth-simaai` is not present, run `ip addr` on the DevKit and match on `10.0.0.2/24` and the MAC ending `:41:31`. Use `ip addr` rather than `ip -br addr` here: the brief form prints only interface, state and IP addresses, so the MAC is not shown.
   :::
