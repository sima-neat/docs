...
2. `ifconfig` output on the SiMa.ai DevKit:
   ...
   :::note
   Older pre-eLxr builds name this interface `eth2`. If `veth-simaai` is not present, run `ip -br addr` on the DevKit and match on `10.0.0.2/24` and the MAC ending `:41:31`.
   :::
