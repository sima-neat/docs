...
- **[Update with sima-cli](?method=sima-cli#sima-cli)** — Transfer the firmware image over the network and automatically trigger the update on the DevKit. This method is ideal for field updates.
- **[Update with Boot Image](?method=bootimage#bootimage)** — Create or recover bootable media for the DevKit. Useful for restoring faulty devices or provisioning multiple boot devices.
- **[Update with Net Boot](?method=netboot#netboot)** — Start a local TFTP server and boot the DevKit directly over the network without flashing media. Ideal for recovery, factory bring-up, or rapid kernel/rootfs development.
...
<Tabs groupId="firmware-update-method" queryString="method">
<TabItem value="sima-cli" label="Update with sima-cli" default>
...
:::note
To **downgrade** the DevKit or install a build that is **not the latest release** (e.g. a pre-release `develop` build or an internal engineering image), use the [Net Boot procedure](?method=netboot#netboot) instead — it boots the DevKit from a TFTP server on the host and flashes the target image directly to eMMC.
:::
