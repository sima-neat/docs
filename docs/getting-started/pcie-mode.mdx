...
1. On the Modalix PCIe Card, there are four DIP switches as shown above. Make sure switch #1 is in the `OFF` position.
2. Make sure the Run/Program switch is ON.
3. Power off the host machine, insert the Modalix PCIe Card into the PCIe slot, and power on the host machine.
...
For a detailed view of the PCIe card interfaces, switches, and connectors, refer to the [Modalix PCIe Card](/hardware/devkit/modalix-pcie-card).
...
:::note
The Virtual Network feature is available in firmware version `1.5` and later on the Linux host. To use this feature, make sure your DevKit is [updated](/hardware/getting-started/firmware-update/) to the latest firmware version.

This feature is only available for a Linux host machine.
:::

When connected to a host machine via PCI Express (PCIe), the Modalix PCIe Card establishes a virtual Ethernet interface. This interface enables direct network communication between the host machine and the PCIe card over the high-speed PCIe bus.
...
- **Assign static IP Address (Ethernet over PCIe)**: Manually configure static IP addresses for your host machine and Modalix PCIe Card.

By default, the `10.0.cardnum.1` address is assigned to the host machine and `10.0.cardnum.2` is assigned to the Modalix PCIe Card. However, for stable and consistent networking, you must disable the default IP address being assigned to the host interface by passing the module parameter `default_ip=false` when loading the module.
...
When loading the module manually (using `insmod`), use the command below. This allows you to manually assign a static IP address to the host machine interface and, in turn, to the Modalix PCIe Card.
