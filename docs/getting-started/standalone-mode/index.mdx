...
- Plug the DevKit into the Ethernet port of the host machine. Use a USB-to-Ethernet dongle if the host machine does not have an Ethernet port available.
...
1. Follow the instructions below for an Ubuntu host machine. On other platforms, the instructions may vary; please look for related instructions online on how to share network connections.
   ...
   - Your host machine should now be networked with the DevKit. `10.42.0.0/24` is the default subnet used by Ubuntu for this setup, but your host machine may use a different subnet.
   - If the next step returns no results, run `ifconfig` and look for the IP of your Ethernet interface (usually `eth0`) there.

2. Open a terminal on the Ubuntu host machine. Type the following to scan the devices in that IP range:
...
:::note
This procedure requires a host machine with the latest `sima-cli` version installed. It also requires a **Modalix DevKit** firmware version that includes the required `.dtbo` files for MIPI support.
:::
